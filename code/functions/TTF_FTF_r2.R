plot_ttf_ftf_r2_comparison <- function(
    r2_data,
    output_file = NULL,
    heatmap_file = NULL,
    combined_file = NULL,
  ...
  context_display <- c(
    BM_single = "BM · univariate",
    BM_multi = "BM · multivariate",
    lambda_single = "lambda · univariate",
    lambda_multi = "lambda · multivariate"
  )
  ...
  if (
    ...
    stop(
      "FTF and TTF results could not be paired.",
      call. = FALSE
    )
  }
  ...
  legend_panel <- ggplot2::ggplot() +
    ...
    ggplot2::geom_text(
      ...
      inherit.aes = FALSE, hjust = 0, size = text_size_mm(0.90),
    ...
    ggplot2::annotate(
      "point", x = 0.735, y = 0.70, shape = 22, size = 2.15,
      stroke = 0.50, fill = "white", color = "grey20"
    ) +
    ggplot2::annotate(
      "text", x = 0.758, y = 0.70, label = "FTF",
      hjust = 0, size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
      "point", x = 0.855, y = 0.70, shape = 22, size = 2.15,
      stroke = 0.50, fill = "grey20", color = "grey20"
    ) +
    ggplot2::annotate(
      "text", x = 0.878, y = 0.70, label = "TTF",
      hjust = 0, size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
      "segment", x = 0.715, xend = 0.755, y = 0.28, yend = 0.28,
      linewidth = 0.42, color = direction_colors[["TTF higher"]]
    ) +
    ggplot2::annotate(
      "text", x = 0.765, y = 0.28, label = "TTF > FTF",
      hjust = 0, size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
      "segment", x = 0.845, xend = 0.885, y = 0.28, yend = 0.28,
      linewidth = 0.42, color = direction_colors[["FTF higher"]]
    ) +
    ggplot2::annotate(
      "text", x = 0.895, y = 0.28, label = "TTF < FTF",
      hjust = 0, size = text_size_mm(), color = "grey25"
    ) +
  ...
  heatmap_plot <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      title = expression(Delta * R^2 * " summary: TTF minus FTF"),
      subtitle = paste0(
  ...
  combined_panel_a <- (
    make_panel_label(
      "A  FTF versus TTF R² performance"
    ) /
      dumbbell_body /
      legend_panel
  ) +
  ...
  summary_rows <- lapply(
    ...
    function(indices) {
      ...
      data.frame(
        ...
        BM_single_delta = current$delta_r2[[1L]],
        BM_multi_delta = current$delta_r2[[2L]],
        lambda_single_delta = current$delta_r2[[3L]],
        lambda_multi_delta = current$delta_r2[[4L]],
        mean_delta = mean(current$delta_r2),
        ttf_higher_count = sum(current$delta_r2 > 1e-12),
        ftf_higher_count = sum(current$delta_r2 < -1e-12),
        stringsAsFactors = FALSE
      )
    }
  )
