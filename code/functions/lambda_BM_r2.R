plot_lambda_bm_r2_comparison <- function(
    r2_data,
    output_file = NULL,
    heatmap_file = NULL,
    combined_file = NULL,
  ...
  comparison_low <- "BM"
  comparison_high <- "lambda"
  context_model_display <- c(BM = "FTF", lambda = "TTF")
  ...
  configuration_map <- c(
    ...
    lambda_FTF = "BM_TTF",
    BM_TTF = "lambda_FTF",
    lambda_TTF = "lambda_TTF"
  )
  original_configuration <- as.character(r2_data$configuration_label)

  if (!all(original_configuration %in% names(configuration_map))) {
    stop(
      "Unexpected configuration_label values were found.",
      call. = FALSE
    )
  }

  r2_data$configuration_label <- unname(
    configuration_map[original_configuration]
  )
  ...
  context_display <- c(
    BM_single = paste0(context_model_display[["BM"]], " · univariate"),
    BM_multi = paste0(context_model_display[["BM"]], " · multivariate"),
    lambda_single = paste0(context_model_display[["lambda"]], " · univariate"),
    lambda_multi = paste0(context_model_display[["lambda"]], " · multivariate")
  )
  ...
  r2_data$model <- ifelse(
    grepl("^BM_", r2_data$configuration_label),
    "BM",
    "lambda"
  )
  r2_data$pheno_error <- ifelse(
    grepl("_TTF$", r2_data$configuration_label),
    "TRUE",
    "FALSE"
  )

  ftf_data <- r2_data[
    r2_data$pheno_error == "FALSE",
    c(
      "group",
      "trait",
      "analysis",
  ...
  names(ftf_data)[names(ftf_data) == "r2"] <- "ftf_r2"

  ttf_data <- r2_data[
    r2_data$pheno_error == "TRUE",
    c(
      "group",
      "trait",
      "analysis",
  ...
  names(ttf_data)[names(ttf_data) == "r2"] <- "ttf_r2"
  ...
  if (
    ...
    anyNA(comparison_data$ftf_r2) ||
    anyNA(comparison_data$ttf_r2)
  ) {
    stop(
      "BM and lambda results could not be paired.",
      call. = FALSE
    )
  }

  comparison_data$delta_r2 <- (
    comparison_data$ttf_r2 - comparison_data$ftf_r2
  )
  ...
  comparison_data$context_key <- paste0(
    comparison_data$model,
    "_",
    ifelse(comparison_data$analysis == "Single trait", "single", "multi")
  )
  comparison_data$context_display <- unname(
    context_display[comparison_data$context_key]
  )
  ...
  context_rank <- match(
    comparison_data$context_key,
    names(context_display)
  )

  comparison_data <- comparison_data[
    order(group_rank, trait_rank, context_rank),
    ,
    drop = FALSE
  ]
  ...
  comparison_data$ftf_plot <- compress_negative(
    comparison_data$ftf_r2
  )
  comparison_data$ttf_plot <- compress_negative(
    comparison_data$ttf_r2
  )

  positive_maximum <- max(
    0.1,
    comparison_data$ftf_r2[comparison_data$ftf_r2 >= 0],
    comparison_data$ttf_r2[comparison_data$ttf_r2 >= 0],
    na.rm = TRUE
  )
  ...
  negative_minimum <- min(
    0,
    comparison_data$ftf_plot,
    comparison_data$ttf_plot,
    na.rm = TRUE
  )
  ...
  comparison_data$ftf_label <- ifelse(
    comparison_data$ftf_r2 < 0,
    sprintf("%.2f", comparison_data$ftf_r2),
    ""
  )
  comparison_data$ttf_label <- ifelse(
    comparison_data$ttf_r2 < 0,
    sprintf("%.2f", comparison_data$ttf_r2),
    ""
  )

  direction_colors <- c(
    `lambda higher` = "#2878B5",
    `BM higher` = "#D55E00",
    Equal = "#6B6B6B"
  )
  ...
  context_key_data <- data.frame(
    x = c(0.205, 0.365, 0.205, 0.365),
    label_x = c(0.228, 0.388, 0.228, 0.388),
    y = c(0.65, 0.65, 0.35, 0.35),
    context = names(context_display),
    label = unname(context_display),
    stringsAsFactors = FALSE
  )

  legend_panel <- ggplot2::ggplot() +
    ggplot2::annotate(
      "rect", xmin = 0.002, xmax = 0.998, ymin = 0.08, ymax = 0.92,
      fill = "white", color = "grey72", linewidth = 0.45
    ) +
    ggplot2::annotate(
      "segment", x = c(0.17, 0.50, 0.67), xend = c(0.17, 0.50, 0.67),
      y = 0.12, yend = 0.88, color = "grey84", linewidth = 0.30
    ) +
    ggplot2::annotate(
      "text", x = 0.086, y = 0.50, label = "Context shape",
      hjust = 0.5, vjust = 0.5,
      size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::geom_point(
      data = context_key_data,
      ggplot2::aes(x = x, y = y, shape = context),
    ...
    ggplot2::annotate(
      "text", x = 0.585, y = 0.70, label = "Endpoint fill",
      hjust = 0.5, vjust = 0.5,
      size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
    ...
    ggplot2::annotate(
      "point", x = 0.720, y = 0.70, shape = 22, size = 2.15,
      stroke = 0.50, fill = "white", color = "grey20"
    ) +
    ggplot2::annotate(
      "text", x = 0.742, y = 0.70, label = comparison_low,
      hjust = 0, vjust = 0.5,
      size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
      "point", x = 0.845, y = 0.70, shape = 22, size = 2.15,
      stroke = 0.50, fill = "grey20", color = "grey20"
    ) +
    ggplot2::annotate(
      "text", x = 0.867, y = 0.70, label = comparison_high,
      hjust = 0, vjust = 0.5,
      size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
      "segment", x = 0.690, xend = 0.730, y = 0.28, yend = 0.28,
      linewidth = 0.42, color = direction_colors[["lambda higher"]]
    ) +
    ggplot2::annotate(
      "text", x = 0.738, y = 0.28,
      label = paste(comparison_high, ">", comparison_low),
      hjust = 0, vjust = 0.5,
      size = text_size_mm(), color = "grey25"
    ) +
    ggplot2::annotate(
      "segment", x = 0.855, xend = 0.895, y = 0.28, yend = 0.28,
      linewidth = 0.42, color = direction_colors[["BM higher"]]
    ) +
    ggplot2::annotate(
      "text", x = 0.903, y = 0.28,
      label = paste(comparison_high, "<", comparison_low),
    ...
    ggplot2::scale_shape_manual(values = context_shapes, guide = "none") +
  ...
  dumbbell_plot <- (
    ...
    patchwork::plot_annotation(
      title = "Phenotypic-error modelling and predictive performance (R² and ΔR²)",
      subtitle = NULL,
      theme = ggplot2::theme(
        plot.title = ggplot2::element_text(
          face = "plain",
  ...
  comparison_data$model <- factor(
    comparison_data$model,
    levels = c("lambda", "BM"),
    labels = unname(context_model_display[c("lambda", "BM")])
  )
  comparison_data$analysis_display <- factor(
  ...
  make_heatmap_group <- function(group_name) {
    ...
    current$heatmap_x <- as.numeric(current$analysis_display)
  ...
  heatmap_key_data <- expand.grid(
    analysis_display = factor(
      c("Univariate", "Multivariate"),
      levels = c("Univariate", "Multivariate")
    ),
    model = factor(
      unname(context_model_display[c("lambda", "BM")]),
      levels = unname(context_model_display[c("lambda", "BM")])
    )
  )

  heatmap_layout_key <- ggplot2::ggplot(
    heatmap_key_data,
    ggplot2::aes(x = analysis_display, y = model)
  ) +
    ggplot2::geom_tile(
      fill = "white",
      color = "grey78",
  ...
  heatmap_plot <- patchwork::wrap_plots(
    ...
    patchwork::plot_annotation(
      title = paste0(
        "ΔR² summary: ", comparison_high, " minus ", comparison_low
      ),
      subtitle = paste0(
  ...
  combined_panel_a <- (
    make_panel_label(
      paste0(
        "A  ", comparison_low, " versus ", comparison_high,
        " R² performance"
      )
    ) /
      dumbbell_body /
  ...
  combined_panel_b <- patchwork::wrap_plots(
    make_panel_label(
      paste0(
        "B  ΔR² summary (ΔR² = ", comparison_high,
        " R² − ", comparison_low, " R²)"
      )
    ),
  ...
  names(detailed_table)[names(detailed_table) == "model"] <- "pheno_error"
  names(detailed_table)[names(detailed_table) == "ftf_r2"] <- "bm_r2"
  names(detailed_table)[names(detailed_table) == "ttf_r2"] <- "lambda_r2"
  ...
  summary_rows <- lapply(
    ...
    function(indices) {
      ...
      current <- current[
        match(names(context_display), current$context_key),
        ,
        drop = FALSE
      ]
      ...
      data.frame(
        ...
        FTF_single_delta = current$delta_r2[[1L]],
        FTF_multi_delta = current$delta_r2[[2L]],
        TTF_single_delta = current$delta_r2[[3L]],
        TTF_multi_delta = current$delta_r2[[4L]],
        mean_delta = mean(current$delta_r2),
        lambda_higher_count = sum(current$delta_r2 > 1e-12),
        BM_higher_count = sum(current$delta_r2 < -1e-12),
        stringsAsFactors = FALSE
      )
    }
  )
